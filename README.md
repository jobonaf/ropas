...
[![Codecov test coverage](https://codecov.io/gh/jobonaf/ropas/graph/badge.svg)](https://app.codecov.io/gh/jobonaf/ropas)
[![api-contract](https://github.com/jobonaf/ropas/actions/workflows/api-contract.yaml/badge.svg)](https://github.com/jobonaf/ropas/actions/workflows/api-contract.yaml)
<!-- badges: end -->

`ropas` is an R client for the [OPAS](https://opas.isprambiente.it) (OPen Air System) REST API.
...
- pre-computed annual statistics and regulatory limit reference tables.

OPAS is used by some Italian environmental agencies. `ropas` does not assume that OPAS represents the full official national air-quality network; it simply provides an R interface to the available API endpoints.
...
Logout clears local authentication state and attempts server-side logout on a best-effort basis:
...
The original API column `measure_value` is replaced by `value_raw` for a cleaner interface.
...
The `post_validity_code` column is the primary field for basic quality filtering.
...
Other validity-related fields such as `measure_code`, `auto_validity_code`, `station_code`, and `final_validity_code` are preserved in the output when provided by the API.

## Timestamps

`measure_date_time` strings returned by OPAS measurement endpoints do not carry an explicit timezone offset.

They are parsed by `ropas` as Italian standard time, i.e. UTC+1 fixed with no daylight saving adjustment, using:
...
For daily aggregates (`last_days` or `daily = TRUE`), records are timestamped at midnight (`00:00:00`) of the reference day.

Synchronisation timestamps such as `measure_insert_ts` and `measure_update_ts` are parsed as UTC when returned by the corresponding API endpoints.
...
Region-level synchronisation can return very large results. Prefer recent timestamps or station-level synchronisation for operational pipelines.

## Annual statistics

Annual pre-computed statistics can be retrieved for stations or individual series.
...
The statistics endpoints return pre-computed results and, when applicable, regulatory limit information such as threshold values and exceedance counts.
...
The package currently prioritises faithful access to OPAS API endpoints. Higher-level helpers for automatic joins, unit conversion, quality filtering, and reporting workflows may be added in future versions.
